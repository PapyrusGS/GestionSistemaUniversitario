...
BEGIN
    ...
    DECLARE v_semestre VARCHAR(10);

    SET v_busqueda = NULLIF(TRIM(p_busqueda), '');
    SET v_semestre = NULLIF(TRIM(p_semestre), '');
    ...
    WHERE (p_idCarrera IS NULL OR m.idCarrera = p_idCarrera)
      AND (v_semestre IS NULL OR m.semestre COLLATE utf8mb4_unicode_ci = v_semestre COLLATE utf8mb4_unicode_ci)
      AND (
            v_busqueda IS NULL
            OR m.idMateria COLLATE utf8mb4_unicode_ci LIKE CONCAT('%', v_busqueda, '%')
          )
    ORDER BY m.estado DESC, m.idCarrera, CASE WHEN m.semestre = 'Electiva' THEN 99 ELSE CAST(m.semestre AS UNSIGNED) END, m.nombre;
END
SQL);
