...
<head>
<meta charset="UTF-8">
<title>Ocupacion de Cursos</title>
<style>
  @page { margin: 2cm; }
  ...
  .pct-baja   { font-weight: 600; }
  .pct-media  { font-weight: 700; }
  .pct-alta   { font-weight: 700; text-decoration: underline; }
  .pct-critica { font-weight: 900; font-style: italic; text-decoration: underline; }
...
<div class="header">
  ...
  <div class="header-report">Ocupacion de Cursos por Periodo</div>
...
<table>
  ...
  <tbody>
    @forelse($data as $row)
    @php
      $pct = (float) rtrim($row[8], '%');
      $cls = $pct >= 90 ? 'pct-critica' : ($pct >= 60 ? 'pct-alta' : ($pct >= 30 ? 'pct-media' : 'pct-baja'));
    @endphp
    <tr>
      <td>{{ $row[0] }}</td>
      <td>{{ $row[1] }}</td>
      ...
      <td class="num"><span class="{{ $cls }}">{{ $row[8] }}</span></td>
    </tr>
    @empty
    <tr><td colspan="9" style="text-align:center;padding:20px;color:#666;">Sin cursos registrados.</td></tr>
    @endforelse
  </tbody>
</table>
