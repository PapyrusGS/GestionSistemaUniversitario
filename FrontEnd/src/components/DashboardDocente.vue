...
const totalEstudiantesMax = computed(() => {
  return cursos.value.reduce((acc, c) => acc + (c.maxInscritos || 0), 0)
})
  ...
  <div class="premium-dashboard-container">
    ...
    <aside class="sidebar-navigation">
      <div class="sidebar-header">
        ...
        <span class="brand-txt">SISTEMA U</span>
      </div>
    ...
    <main class="main-workspace">
      <header class="workspace-topbar">
        <div class="topbar-left">
          <span class="context-path">Dashboard / Docentes</span>
          <h2>Panel de Gestión Escolar</h2>
        </div>
        <div v-if="errorMessage" class="alert-inline error">{{ errorMessage }}</div>
      </header>
      ...
      <section class="workspace-body">
        ...
        <div v-if="modoDocente === 'inicio'" class="fade-in-view">
          <div class="welcome-banner">
            <h3>¡Qué bueno verte de nuevo, Docente!</h3>
            <p>Monitorea y descarga las listas de asistencia vigentes y planifica el cupo asignado para el periodo académico.</p>
          </div>

          <h4 class="block-title">Resumen Informativo</h4>
          <div class="dashboard-cards-grid">
            <div class="metric-card">
              ...
              <div class="metric-info">
                <span class="lbl">Materias Dictadas</span>
            ...
            <div class="metric-card">
              ...
              <div class="metric-info">
                <span class="lbl">Cupos Máximos</span>
                <strong class="val">{{ totalEstudiantesMax }} Estudiantes</strong>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="modoDocente === 'cursos'" class="fade-in-view">
          <div class="table-card-wrapper">
            <div class="table-card-header">
              <h4>Listado de Cursos y Horarios Asignados</h4>
            </div>
              ...
              <table class="workspace-table">
                ...
                <tbody>
                  <tr v-for="curso in cursos" :key="curso.idCursoMateria">
                    <td class="primary-cell">{{ curso.materia_nombre || 'Materia sin Nombre' }}</td>
                    ...
                    <td class="font-medium">{{ curso.maxInscritos }} alumnos</td>
                    <td class="txt-right">
                      <button class="action-row-btn" @click="verDetalleCurso(curso)">Ver Alumnos</button>
                    </td>
                  </tr>
                  <tr v-if="cursos.length === 0">
                    <td colspan="5" class="empty-table-msg">No se encontraron materias asignadas para su usuario.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="table-card-wrapper">
            ...
            <div class="table-responsive">
              <table class="workspace-table">
                ...
                <tbody>
                  <tr v-for="estudiante in estudiantesInscritos" :key="estudiante.id_estudiante">
                    <td class="primary-cell font-mono">{{ estudiante.ci }}</td>
                    <td class="font-medium">{{ estudiante.apellido1 }} {{ estudiante.apellido2 }}, {{ estudiante.nombre1 }}</td>
                    <td class="text-muted">{{ estudiante.correo }}</td>
                    <td class="text-muted">{{ new Date(estudiante.fecha_inscripcion).toLocaleDateString() }}</td>
                  </tr>
                  <tr v-if="estudiantesInscritos.length === 0">
                    <td colspan="4" class="empty-table-msg">No hay estudiantes inscritos en esta materia actualmente.</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <aside class="sidebar-profile-panel">
      ...
      <div class="quick-status-widget">
        ...
        <div class="widget-row">
          ...
          <span class="value text-ellipsis">{{ user?.correo }}</span>
        </div>
        <div class="widget-row">
...
.m-blue { background: rgba(56, 189, 248, 0.1); color: #38bdf8; }
.m-purple { background: rgba(168, 85, 247, 0.1); color: #a855f7; }
.metric-info { display: flex; flex-direction: column; }
.metric-info .lbl { font-size: 0.75rem; color: #64748b; text-transform: uppercase; }
.metric-info .val { font-size: 1.1rem; font-weight: 700; color: #f1f5f9; margin-top: 0.1rem; }
...
.action-back-btn svg { width: 16px; height: 16px; }
.action-back-btn:hover { color: #38bdf8; }
.header-title-group { display: flex; align-items: center; gap: 0.75rem; }
.sub-badge { font-size: 0.7rem; background: rgba(168, 85, 247, 0.15); color: #c084fc; padding: 0.15rem 0.4rem; border-radius: 4px; font-weight: 700; }
...
.profile-meta h5 { font-size: 0.95rem; font-weight: 600; color: #f8fafc; }
.user-role-tag { font-size: 0.7rem; color: #64748b; font-weight: 600; text-transform: uppercase; }
.separator-line { border: 0; border-top: 1px solid rgba(255, 255, 255, 0.05); margin: 2rem 0; }

.quick-status-widget h6 { font-size: 0.8rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 1rem; }
.widget-row { display: flex; flex-direction: column; gap: 0.2rem; margin-bottom: 1rem; }
.widget-row .label { font-size: 0.75rem; color: #475569; }
...
.alert-inline { padding: 0.5rem 1rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; background: rgba(239, 68, 68, 0.1); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.2); }
</style>
